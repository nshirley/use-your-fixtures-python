...
def test_auth_rejects_without_username_password_with_fixture(unauthorized_session):
    expected_response = {"error": "bad request", "details": "username and password are required"}
...
def test_auth_rejects_with_incorrect_username_password_with_fixture(unauthorized_session):
    ...
    response = unauthorized_session.post(f'{BASE_URL}{ROUTES.AUTH}', json={'username': 'user', 'password': 'wrong_password'})
